...
```json
{
  "text": "Описание сцены, опиши детально, что происходит с игроком, где он находится и кто или что есть вокруг него",
...
def build_prompt(state: GameState) -> GameState:
    ...
    При новой игре (is_new_game) использует START_PROMPT из ноды start_game,
    который содержит инструкцию на генерацию персонажа, мира и первой сцены.
    ...
    """
    # При новой игре используем START_PROMPT из start_game
    if state.is_new_game:
        start_prompt = state.extra.get("start_prompt")
        if start_prompt:
            state.prompt = start_prompt
            logger.debug("Prompt (новая игра): %d символов (START_PROMPT)", len(state.prompt))
            return state
